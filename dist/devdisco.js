...
function help() {
  console.log(['Usage: devdisco <cmd>', '', 'where <cmd> is one of:', '    list, update, upgrade, version', '', 'Sample:', '', 'devdisco list --eth      list the Ethernet devices', 'devdisco list --usb      list the USB serial devices', 'devdisco list --wifi     list the Wi-Fi devices', ''].join('\r\n'));
  ...
  for (var i = 0; i < argv.length; i++) {
    switch (argv[i]) {
      ...
      case '--wifi':
        ...
        break;
      case '--timeout':
        // TODO: add documentation, we also need to check if multiple -timeout values have been specified
...
var deviceStatus = {};
var onDeviceDiscovered = function onDeviceDiscovered(record) {
  var deviceInfoStr = '?';
  ...
  switch (record.transport) {
    case 'ethernet':
      ...
      if (record.force_process || record.mac_address && record.host_name) {
        ...
        console.log(rpad(record.ip_address, 16), rpad(record.mac_address || '??:??:??:??:??:??', 20), rpad(deviceInfoStr || '?', 32), record.host_name ? record.host_name.join(' ') : '');
      }
      break;
    case 'wifi-ap':
      console.log(rpad(record.ssid, 16), rpad(record.mac_address || '??:??:??:??:??:??', 20), rpad(deviceInfoStr || '?', 32), rpad(record.channel, 8), rpad(record.signal_level, 14));
      break;
    case 'usb-uart':
      console.log(rpad(record.com_name, 28), rpad(record.device_name_or_manufacturer, 40), rpad(deviceInfoStr || '?', 46));
      break;
    default:
...
function cmd_list_ethernet(timeout) {
  ...
  Timer.timeoutAsync(timeout).then(function () {
    // stop after timeout
